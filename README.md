...
基于 STM32F103C8 的舵机 PWM 测试小程序，用于生成 PWM 信号驱动舵机并验证角度控制。支持 **B11 单按键循环跳变**与**旋转编码器微调**双控模式。
...
| 操作 | 效果 |
|------|------|
| 按 **B11** (PB11) | 循环跳变：0→30→60→90→120→150→180→150→120→90→60→30→… |
| 顺时针旋转编码器 | 角度 +1°/脉冲 |
| 逆时针旋转编码器 | 角度 -1°/脉冲 |
| 按键后转编码器 | 从按键档位出发微调 |
...
- **单键循环**：B11 按固定序列 0→30→60→90→120→150→180→150→...→30 循环跳变
...
Key1 (PA4) 在软件层面已不再使用，仅保留 GPIO 初始化。当前仅 B11 (PB11) 作为唯一操作按键。
